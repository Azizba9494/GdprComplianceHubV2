...
- July 11, 2025. Enhanced Risk Assessment Interface with AI Progress Indicators - completely redesigned DPIA risk assessment section to match other modules' design with one question per line, replaced all 9 simple Button components with AIProgressIndicator components featuring 35-second progress tracking and detailed step descriptions, standardized layout from 2-column grid to single-column space-y-6 design for better consistency, improved field descriptions and placeholder text with specific examples for impacts, sources, and measures across all three risk scenarios (illegitimate access, data modification, data disappearance)
- July 11, 2025. Fixed AI Progress Indicator Time Display - removed inaccurate time remaining calculation from AIProgressIndicator component, now shows simplified "Génération en cours..." message instead of potentially incorrect elapsed/estimated time counters, maintaining progress bar and step descriptions for user feedback
- July 11, 2025. Fixed DPIA Save Issue - resolved "PayloadTooLargeError" preventing DPIA assessment saves by increasing Express.js JSON and URL-encoded payload limits from default 100KB to 10MB, allowing large AI-generated content to be saved successfully
- July 11, 2025. Harmonized DPIA Risk Assessment Prompts - resolved JSON output issue in risk assessment sections by completely harmonizing all 9 prompts in Section 4, eliminated duplicate "Mesures existantes" prompts keeping only professional DPO-style evaluation format, standardized all "Sources" prompts to produce descriptive French text instead of JSON structures, ensuring consistent user experience across all risk analysis modules
- July 11, 2025. CRITICAL FIX: AI Field Name Contamination Resolution - resolved issue where AI responses occasionally started with technical field names (illegitimateAccessImpacts, dataModificationSources, etc.) instead of proper content by adding explicit anti-contamination rules in Gemini service prompts, implemented comprehensive frontend cleaning functions to automatically remove technical field prefixes from AI responses, added detection for 24 different technical field names across all DPIA modules ensuring clean professional French output in all cases
